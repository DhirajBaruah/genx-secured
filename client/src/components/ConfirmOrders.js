...
const selectedAddress = (addressLine1,addressLine2,city,state,pinCode,nation) => {
  ...
  useEffect(()=> {
      ...
      axios.post(`/getDataOfAddresses/${props.user._id}`,{
      ...
      .then(response =>{
          ...
          for(var i=0; i<response.data.length; i++){
                ...
                element.push(<React.Fragment><CardAddress 
    ...
    axios.post(`/getDataOfProducts/${props.match.params.productId}`,{
                ...
                element2.push(
                    <React.Fragment>
                        ...
                        <div className="col s4">
                            <img className="responsive-img"   src={`/images/${response2.data[0].productName}1.jpg`}/>
                        </div>
                        <div className="col s4">
    ...
    const addToCart = () => {
        ...
        axios.post('/addToCart',formData,(req,res)=>{

        })
        .then(res=>{
            console.log(res);
            if(res.status==200){
                 props.history.push(`/modeOfPayment/${props.user._id}/${AddressId}/${props.match.params.productId}/${qty}/${total}`);
            }
        })
    }
       ...
       switch(props.user){
           ...
           default:
                ...
                return(
                   ...
                   <h4>{AddressId}</h4>
                   <p>
                    {selectedAddressLine1}<br/>
                    {selectedAddressLine2}<br/>
                    {selectedCity}<br/>
                    {selectedState} - {selectedPinCode}<br/>
                    {selectedNation}<br/>
                   </p>

                  
                   <button  onClick={() => setShowModal(true)}>Add/Select Address</button>
                  
                  {ProductData}

                    <h4>{total}</h4>
                     <input type="number" value={qty} onChange={(e) =>{subTotal(e.target.value,price)}}/>
                   
                   <button onClick={()=>{if(AddressId==''){alert('Please select address.')}else{addToCart()}}}>Continue</button>
